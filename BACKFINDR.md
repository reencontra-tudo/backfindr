...
- Workflow AutoPost Facebook: **a cada 5 dias** (node "Cron 4h" — nome desatualizado, configurado com `daysInterval: 5`; confirmado ao vivo em 31/08/2026), **6 nichos** (pet/celular/veiculo/bicicleta/geral/protect), gpt-image-1 → R2 → Instagram → auditoria de imagem (gpt-4o-mini vision) → Facebook. A doc antiga dizia "4 posts/dia, 7 nichos" e "2x/dia" (ver histórico de sessão 21/07) — estava desatualizada em ambos os pontos; cadência real não foi alterada, só a documentação foi corrigida.
...
- **⚠️ 5 tokens Meta em texto puro nos nodes HTTP do workflow (não no cofre de credenciais do n8n) — tratar como comprometidos.** Confirmados válidos e funcionando em 28/08/2026 (execução real retornou `post_id`); a doc antiga dizia que expiravam ~30/07/2026, isso estava errado. Pendência completa e ação necessária: ver seção 17, 🔴 Alto.
...
### 🔴 Alto
- **5 tokens Meta hardcoded em texto puro nos nodes HTTP do workflow n8n "Backfindr AutoPost — Facebook" (achado 31/08/2026, NÃO corrigido)** — 4 tokens de página do Facebook (pet/celular/veiculo/bicicleta, mais o compartilhado geral+protect) e 1 token de usuário do Instagram estão escritos direto nos parâmetros dos nodes `HTTP Request1`, `HTTP Request4`, `HTTP Request5`, em vez de estarem no cofre de credenciais do n8n. **Confirmados válidos e funcionando** (execução real de 28/08/2026 retornou `post_id`) — a doc antiga (seção 12) dizia que expiravam ~30/07/2026, essa data estava errada/desatualizada. Tratar como comprometidos mesmo funcionando: mover pro cofre de credenciais do n8n + gerar tokens novos. Não editado ainda — é automação viva em produção (posta a cada 5 dias), mexer sem pedido explícito é risco de derrubar o AutoPost.
...
- **Ciclo "Encontrei" — cron agendado no n8n, falta colar o `CRON_SECRET` real**: workflow "Backfindr Found Pending — Lembretes e Timeout" (id `hDJeRz5YpWi673u1`) criado e **ativado** em 26/08/2026 via API REST do n8n (canvas visual estava com falha intermitente de clique sintético na sessão) — Schedule Trigger diário às 2am → `GET https://backfindr.com/api/v1/cron/found-pending-reminders` com header `Authorization`. O valor do header ficou como placeholder (`Bearer COLOQUE_O_CRON_SECRET_AQUI`) porque `CRON_SECRET` é uma env var tipo Secret no Vercel — write-only, não dá pra revelar o valor salvo pela UI. **Pendência real**: abrir o node "HTTP Request" no n8n e colar o valor verdadeiro de `CRON_SECRET` (mesmo já usado em `cron/lifecycle`) no lugar do placeholder — até lá, a execução diária das 2am vai falhar com 401, sem risco (só não faz nada).
...
- **Incidente de segurança: credenciais R2 vazadas em repo público, investigadas e corrigidas (31/08/2026)** — `src/lib/storage.ts` tinha o Account ID, Access Key ID e Secret Access Key reais do R2 de produção em texto puro no comentário do cabeçalho, num repo **público** no GitHub. Código corrigido: PR [#29](https://github.com/reencontra-tudo/backfindr/pull/29) (commit `cdb996e`, merge `0336d549`) remove os valores reais, deixa só nomes de env var como documentação. **Investigação revelou que o token vazado (`backfindr-r2-production`) não era o que estava de fato configurado em produção** — o Vercel usa um token Cloudflare diferente (`backfindr-r2-manus`, ver comentário em `storage.ts`). O token vazado foi deletado direto na Cloudflare (sem impacto — nada dependia dele); um token substituto criado por engano durante a resposta ao incidente (secret perdido antes de ser copiado, nunca chegou a ser usado em lugar nenhum) também foi deletado. Histórico completo (`git log --all -S<chave>`, todas as branches) dos 12 repos da org `reencontra-tudo` foi verificado pela chave que está *realmente* em uso — nenhuma exposição encontrada, então essa chave **não foi rotacionada** por precaução (trocar uma credencial funcionando sem evidência de vazamento é risco desnecessário). **Lição geral** (ver também seção 18): nome de variável de ambiente ou de token documenta a intenção de quem escreveu, não garante qual credencial está de fato configurada — sempre conferir o valor real antes de agir sobre uma credencial com base só no nome. Vale checar esse mesmo tipo de descompasso nome↔valor real em outros produtos M2R que têm mais de uma credencial com nomes parecidos (ex.: M2RFood tem um `R2 Account Token` Cloudflare separado do do Backfindr).
- **Ciclo "Encontrei" — mitigação do `/notify` anônimo + timeout do banner (25/08/2026, item 3a+3b)**: `/notify` (rota pública sem autenticação nenhuma) mudava `status` do objeto pra `'found'` na hora, sem confirmação do dono — Marcos já acionou isso sem querer. Corrigido com um novo campo isolado, `objects.found_pending_confirmation` (+ `found_pending_since`): `/notify` agora só marca essa flag, nunca toca em `status` — uma sinalização anônima falsa/mal-intencionada não contamina mais o status real, que só muda quando o dono confirma de verdade via `FoundBanner`. Também adicionado cooldown de 15min entre notificações pro mesmo objeto (evita spam de push/e-mail/in-app por scans repetidos). `FoundBanner` reescrito: gatilho `status === 'found'` → `found_pending_confirmation`; "Ainda não recebi" virou `dismissPending` (só limpa a flag, não precisa mais reverter status). Timeout (3a, opção D): novo cron `GET /api/v1/cron/found-pending-reminders` — lembrete D+3 e D+7, descarte automático da sinalização em D+14 (nunca desfaz um retorno real, já que a flag nunca mudou `status`). PR [#14](https://github.com/reencontra-tudo/backfindr/pull/14). Verificado em produção com objeto/conta de teste descartáveis: `/notify` não mudou `status` (continuou `lost`), cooldown funcionou na segunda chamada, `dismissPending` e `confirmReturn` funcionaram isoladamente, e a query de descarte D+14 foi validada diretamente (objeto com `found_pending_since` simulado em 15 dias atrás foi corretamente descartado). **Atualização (26/08/2026)**: cron já agendado e ativo no n8n — falta só colar o valor real do `CRON_SECRET` no node (ver seção 17).
- **`objects.status` — 3 fontes de verdade consolidadas (25/08/2026)**: mapeamento de 23/08 (schema, valores em uso, call sites) resolvido antes de implementar o timeout do FoundBanner e a mitigação do `/notify` anônimo. Contra dados reais (só 4 valores em uso: `stolen`, `lost`, `found`, `returned`), confirmado que `protected` e `archived` são status **reais e distintos** (não sinônimos) — `protected` é proteção preventiva ativada pelo dono (dashboard/matching/`/notify`/onboarding), `archived` é moderação ocultando objeto denunciado (só `admin/moderacao/route.ts` seta isso). Corrigido: `ObjectStatus` (`src/types/index.ts`) ganhou os 6 valores; `VALID_STATUSES` do admin (`api/v1/admin/objects/[id]/route.ts`) ganhou `protected` (faltava, admin não conseguia proteger objeto por ali); PATCH do dono (`api/v1/objects/[id]/route.ts`) ganhou validação contra `OWNER_SETTABLE_STATUSES` (5 valores, sem `archived` de propósito — antes aceitava qualquer string); `STATUS_CONFIG` do dashboard ganhou entrada pra `archived` (evita mostrar objeto oculto como "Perdido"). PR [#13](https://github.com/reencontra-tudo/backfindr/pull/13). Verificado em produção com objeto/conta de teste descartáveis: status inválido → 400, `archived` via rota do dono → 400 (bloqueado corretamente), `protected` → 200.
- **Auditoria de env vars Vercel — 2 bugs de segurança corrigidos (25/08/2026)**: comparação completa Production vs Preview vs `.env.example`/uso real no código. Produção e Preview batem entre si (nenhuma variável só em um dos dois). Achados de segurança, corrigidos com prioridade:
  - `ADMIN_IDS` (allowlist de `api/v1/admin/migrate/route.ts`): lia uma env var que nunca existiu no Vercel (só `ADMIN_USER_IDS`, nome diferente, nunca usada em código) — `ADMIN_IDS.length === 0` sempre verdadeiro fazia a rota aceitar **qualquer JWT válido, não só de admin**. Corrigido pra ler `ADMIN_USER_IDS`. Verificado com JWT real de conta de teste não-admin (criada e removida via SQL): antes indeterminado, depois do fix retorna 403 Forbidden.
  - `WEBHOOK_TOKEN`: 3 rotas (`marketing/webhook`, `marketing/apify-webhook`, `marketing/cleanup`) tinham fallback hardcoded `'backfindr-webhook-secret-2024'` no source — como a var nunca foi configurada no Vercel, rodavam com esse literal público como segredo real. Pior, achado durante a correção: `GET /api/v1/admin/marketing/webhook` não tinha autenticação nenhuma e devolvia o token real em texto puro pra qualquer request, invalidando qualquer rotação por si só. Corrigido: fallback removido (fail closed), GET agora exige `requireAdmin` e nunca ecoa o valor completo, e `WEBHOOK_TOKEN` foi rotacionado no Vercel (valor antigo tratado como comprometido). Verificado em produção: token antigo → 401, sem header → 401, GET sem auth → 401 (antes vazava o token), token novo → 201 funcional.
  - PR [#12](https://github.com/reencontra-tudo/backfindr/pull/12).
  - **Pendente pra depois** (não bloqueante, sinalizado mas não corrigido): `FACEBOOK_APP_ID`/`FACEBOOK_APP_SECRET` ausentes (login social Facebook provavelmente quebrado), `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` ausentes (Stripe parece desativado a favor do Mercado Pago, que está configurado), `ZAPI_*` ausentes (WhatsApp — já rastreado como "Loop WhatsApp" nesta seção), e ~10 vars configuradas no Vercel sem nenhum uso no código atual (`NEXTAUTH_URL`, `NEXTAUTH_SECRET`, `GOOGLE_CSE_ID`, `GOOGLE_API_KEY`, `ADMIN_EMAIL`, `SUPERADMIN_EMAIL`, `ADMIN_USER_IDS`, `CORS_ORIGINS`, `NEXT_PUBLIC_GOOGLE_CLIENT_ID`, `NEXT_PUBLIC_MP_PUBLIC_KEY`) — candidatas a limpeza, resquício de versão anterior (NextAuth substituído pelo JWT custom em `adminGuard.ts`).
- **Instabilidade de cliques na UI de `/admin/public-signals` — monitorado, sem ação de código (25/08/2026)**: reproduzido de novo (timeout do CDP em "Rejeitar", aba travando minutos). Cross-check: mesmo padrão idêntico ocorreu numa aba completamente diferente (SQL Editor do Supabase, outra origem) na mesma sessão, e o clique nunca chegou a produzir efeito no banco (status da evidência não mudou, confirmado via SQL). Sem banner de incidente ativo no Supabase no momento do teste. Conclusão: instabilidade da camada de automação de browser desta sessão, não bug da página nem incidente confirmado de plataforma. Marcos pediu para não investir mais tempo aqui — encerrado como monitorado.
- **Descrição genérica em objetos publicados via Public Signals — correção sistêmica (25/08/2026)**: `buildPublicSignalDescription()` (novo, `src/lib/publicSignals/description.ts`) monta a description a partir de `title`/`category`/`location_text` já validados pela extração — **não** de `raw_description`: amostragem em produção mostrou esse campo como fonte perigosa (institucional às vezes embute telefone de contato do achador — vazaria PII; Google News RSS às vezes vem como fragmento de HTML cru/truncado). Aplicado como correção definitiva na rota de aprovação (`public-signals/route.ts`, PR #10) e via backfill one-off `fix-legacy-public-signal-descriptions/route.ts` (PR #10, auth ajustada no PR #11 pra aceitar sessão admin além do `MIGRATION_SECRET`) rodado contra produção: **86 de 86 objetos corrigidos**, confirmado por reexecução idempotente (0 restantes) e amostragem SQL das descriptions novas.
- **Label "Veículo Roubado" na navegação de categorias corrigido (25/08/2026)**: `CATEGORY_LABELS.veiculo.label` virou "Veículo" nos dois arquivos (`achados-perdidos/[cidade]/page.tsx` e `.../[categoria]/page.tsx`) — a página dedicada de veículo mantém "roubado" no próprio conteúdo (CTA "Meu veículo foi roubado", copy da categoria), só a navegação genérica ("Outras categorias") mudou. Edição original feita numa sessão paralela (task `task_4978c1f2`) e nunca commitada; recuperada do working tree, diff revisado, PR #9 aberto e mergeado (`fcd0216`). Verificado ao vivo em produção nas duas páginas (`/sao-paulo/geral` e `/sao-paulo/veiculo`).
...
- **Nome de variável de ambiente ou de credencial documenta intenção, não garante qual valor está de fato configurado** — antes de agir sobre uma credencial (rotacionar, revogar, confiar nela) com base só no nome, confira o valor real configurado no Vercel/Cloudflare/onde for. Lição do incidente R2 de 31/08/2026 (seção 17): um comentário em `storage.ts` documentava um token que não era o que estava realmente em uso.
...
| 31/08 | Auditoria do motor n8n do AutoPost (pra proposta de Central de Mídia em outro repo) revelou incidente de segurança: credenciais R2 vazadas em `storage.ts`, corrigidas e investigadas a fundo (PR #29) — token vazado confirmado como não sendo o realmente usado em produção, deletado sem impacto; a chave real verificada limpa no histórico de todos os repos da org, não rotacionada por precaução. Achado novo, não corrigido: 5 tokens Meta hardcoded no n8n (seção 17, 🔴 Alto). Cadência real do AutoPost confirmada e documentação corrigida (a cada 5 dias, 6 nichos — não "4 posts/dia, 7 nichos"/"2x/dia" como estava escrito). Ver histórico detalhado abaixo. |
...
### Sessão 31/08/2026 — Incidente de segurança R2 + achados no motor n8n do AutoPost

**Origem:** auditoria somente-leitura do workflow n8n "Backfindr AutoPost — Facebook" (id `urluPuyxe4ccY9ZE`), feita a pedido de uma tarefa em outro repo (`m2rintelligence`, proposta de "Central de Mídia" compartilhada entre produtos M2R — fora do escopo deste documento). A auditoria expôs dois achados de segurança do Backfindr em si, tratados nesta sessão.

**1. Credenciais R2 de produção vazadas em `src/lib/storage.ts` (resolvido — ver seção 17):**
- Achado: Account ID, Access Key ID e Secret Access Key reais do R2 estavam em texto puro no comentário do cabeçalho do arquivo, desde a criação do arquivo em 25/04/2026 (commit `b64a4ed`) — este repo é público no GitHub.
- Corrigido: PR [#29](https://github.com/reencontra-tudo/backfindr/pull/29) (commit `cdb996e`, merge `0336d549`) remove os valores reais, mantém só os nomes das env vars.
- **Descoberta que mudou a ordem de remediação planejada:** comparando o Access Key ID vazado (`3a3361dc...`, token Cloudflare `backfindr-r2-production`) contra o valor de fato configurado no Vercel (`f3bfe3f4...`), os dois **não batiam**. O token realmente em uso em produção é outro, `backfindr-r2-manus` — nome documentado em `storage.ts` desde então (comentário novo, sem valores, com aviso explícito de não confiar só no nome). Isso significou que revogar o token vazado não precisava da dança cuidadosa de gerar substituto → colar no Vercel → testar → só então revogar: nada em produção dependia dele. Token vazado deletado direto na Cloudflare, sem downtime.
- Durante a resposta ao incidente, um token de substituição foi criado por engano (`backfindr-r2-production-rotated-20260831`) antes de confirmar que ele nem seria necessário — a aba do painel Cloudflare foi navegada para outro lugar antes do secret (mostrado uma única vez) ser copiado, perdendo-o. Esse token órfão, sem uso possível, também foi deletado.
- **Busca de exposição da chave real:** os 12 repositórios da organização `reencontra-tudo` foram clonados e verificados com `git log --all -S<chave>` (todos os commits, todas as branches) pela chave completa e por prefixo, além de busca no conteúdo atual e no `BACKFINDR.md`. Nenhuma ocorrência encontrada. Decisão: **não rotacionar** essa chave por precaução — sem evidência de vazamento, trocar uma credencial funcionando é risco desnecessário, não benefício.
- **Lição registrada como princípio geral** (seção 18): nome de variável de ambiente ou de token documenta a intenção de quem escreveu, não garante qual credencial está de fato configurada — sempre conferir o valor real antes de agir sobre uma credencial com base só no nome. Foi exatamente esse descompasso (comentário citando um token, Vercel usando outro) que causou a confusão inicial neste incidente. Vale checar o mesmo tipo de descompasso em outros produtos M2R com mais de uma credencial de nome parecido (ex.: M2RFood tem um `R2 Account Token` Cloudflare separado do do Backfindr).

**2. Tokens Meta hardcoded no workflow n8n (achado, NÃO corrigido — ver seção 17, 🔴 Alto):**
- 5 tokens (4 de página do Facebook + 1 de usuário do Instagram) estão em texto puro nos parâmetros de nodes HTTP do workflow, não no cofre de credenciais do n8n.
- Confirmados válidos e funcionando via execução real de 28/08/2026 (retornou `post_id`) — a doc antiga (seção 12) dizia que expiravam ~30/07/2026, o que está incorreto/desatualizado.
- Não editado — automação viva em produção, mexer sem pedido explícito é risco de derrubar o AutoPost. Ação pendente: mover pro cofre de credenciais + gerar tokens novos, tratando os atuais como comprometidos apesar de funcionando.

**3. Cadência real do AutoPost confirmada e documentação corrigida (seção 12):** node "Cron 4h" (nome desatualizado) está configurado com `daysInterval: 5` — a cada 5 dias, não alterado nesta sessão (cadência real preservada de propósito). A doc antiga dizia "4 posts/dia, 7 nichos" (e um histórico de sessão anterior, 21/07, registrava "2x/dia" — ambos desatualizados). Nichos reais: 6 (pet/celular/veiculo/bicicleta/geral/protect). Sequência real do pipeline também confirmada: texto (gpt-4o-mini) → imagem (gpt-image-1) → upload R2 → publica Instagram → **só depois** auditoria de imagem (gpt-4o-mini vision) → se aprovado, publica Facebook — a auditoria não cobre o Instagram (publica antes de auditar); registrado como observação, não foi pedido para corrigir.
