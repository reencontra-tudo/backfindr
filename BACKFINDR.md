...
- **Ciclo "Encontrei" — cron de lembrete/timeout ainda não agendado**: `GET /api/v1/cron/found-pending-reminders` existe e foi testado manualmente (25/08/2026, ver PR #14), mas ainda não está conectado a nenhum agendador (nem Vercel Cron, nem n8n) — precisa do mesmo passo manual usado pro cron "Backfindr Public Signals — Ingestão Diária" (criar workflow no n8n com Schedule Trigger 1x/dia → `GET` nessa rota com header `Authorization: Bearer $CRON_SECRET`). Sem isso, os lembretes D+3/D+7 e o descarte automático D+14 nunca disparam sozinhos.
