...
from core.rate_limit import login_limit, register_limit, forgot_limit
...
async def register(payload: RegisterPayload, db: AsyncSession = Depends(get_db), _rl=Depends(register_limit)):
...
async def login(form: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_db), _rl=Depends(login_limit)):
