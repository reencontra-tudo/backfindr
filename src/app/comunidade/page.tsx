...
function PostCard({ post, featured = false }: { post: Post; featured?: boolean }) {
  ...
  return (
    <Link href={`/comunidade/${post.slug}`} className={`group block rounded-2xl overflow-hidden border border-gray-800 bg-gray-900 hover:border-teal-500/40 transition-all duration-200 hover:shadow-lg hover:shadow-teal-500/5 ${featured ? 'md:col-span-2' : ''}`}>
      {post.cover_url && (
...
export default function ComunidadePage() {
  ...
  const shareUrl = typeof window !== 'undefined' ? window.location.href : 'https://backfindr.com/comunidade';
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <section className="max-w-6xl mx-auto px-4 pt-12 pb-8">
        <div className="flex items-start justify-between gap-6 flex-wrap">
          ...
          <button
            onClick={() => navigator.share?.({ title: 'Comunidade Backfindr', url: shareUrl }).catch(() => navigator.clipboard?.writeText(shareUrl))}
