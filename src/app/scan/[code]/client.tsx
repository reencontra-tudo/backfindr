...
import {
  ...
  Gift, ChevronRight, Shield, Clock,
  Heart, Zap, Eye, UserPlus
} from 'lucide-react';
...
const STATUS_CONFIG: Record<string, { label: string; dot: string; pill: string }> = {
  ...
  protected: { label: 'Protegido',     dot: 'bg-blue-400',   pill: 'bg-blue-500/90 text-white' },
};

// ─── Configuração de CTA por status ──────────────────────────────────────────
// Define o que o achador/observador vê dependendo do estado do objeto
const CTA_CONFIG: Record<string, {
  buttonLabel: string;
  ...
  successSub: string;
  successTimeline: { n: string; title: string; sub: string }[];
  showChatInvite: boolean; // convidar a criar conta para abrir chat
}> = {
  lost: {
    buttonLabel: 'Encontrei este objeto!',
    buttonIcon: <MessageCircle className="w-5 h-5" />,
    buttonColor: 'linear-gradient(135deg, #00d4aa 0%, #00b894 100%)',
    footerText: 'O dono será notificado anonimamente · Backfindr',
    successHeadline: 'Boa ação registrada!',
    successSub: 'O dono foi notificado. Obrigado por ajudar — isso faz diferença de verdade. 🙏',
    successTimeline: [
      { n: '1', title: 'Dono recebe notificação agora',    sub: 'Via push, email ou SMS — dependendo das configurações dele.' },
      { n: '2', title: 'Dono entra em contato',            sub: 'Ele responde pelo Backfindr — seu número fica protegido.' },
      { n: '3', title: 'Vocês combinam a devolução',       sub: 'Sempre em local público e seguro — a plataforma orienta os dois.' },
    ],
    showChatInvite: true,
  },
  protected: {
  ...
  stolen: {
    buttonLabel: 'Vi este objeto — avisar o dono',
    buttonIcon: <Eye className="w-5 h-5" />,
    buttonColor: 'linear-gradient(135deg, #f97316 0%, #ea580c 100%)',
    footerText: 'Sua identidade não será revelada · Backfindr',
    successHeadline: 'Aviso enviado ao dono!',
    successSub: 'Este objeto foi reportado como roubado. Seu aviso foi registrado e o dono foi notificado.',
    successTimeline: [
      { n: '1', title: 'Dono recebe o aviso agora',        sub: 'Ele saberá que alguém viu o objeto.' },
      { n: '2', title: 'Dono pode acionar as autoridades', sub: 'Com base na localização do scan, se fornecida.' },
      { n: '3', title: 'Sua identidade fica protegida',    sub: 'O Backfindr preserva seu anonimato em todos os casos.' },
    ],
    showChatInvite: false,
  },
  found: {
    buttonLabel: 'Este é meu objeto!',
    buttonIcon: <CheckCircle2 className="w-5 h-5" />,
    buttonColor: 'linear-gradient(135deg, #00d4aa 0%, #00b894 100%)',
    footerText: 'Identifique-se pelo Backfindr · seguro e anônimo',
    successHeadline: 'Identificação registrada!',
    successSub: 'Sua reivindicação foi enviada. O sistema entrará em contato.',
    successTimeline: [
      { n: '1', title: 'Verificação em andamento',         sub: 'O Backfindr cruza suas informações com o registro original.' },
      { n: '2', title: 'Confirmação de titularidade',      sub: 'Perguntas sobre o objeto confirmam que é seu.' },
      { n: '3', title: 'Combinam a devolução',             sub: 'Chat mediado entre quem achou e você.' },
    ],
    showChatInvite: true,
  },
};
...
export default function ScanPage() {
  ...
  const notifyOwner = async () => {
    ...
    try {
      ...
      setContactSent(true); // mostra sucesso mesmo em erro — não bloquear o achador
    } finally {
      setSending(false);
    }
  };
  ...
  // Configuração de CTA baseada no status — fallback para 'lost'
  const cta = CTA_CONFIG[obj.status] ?? CTA_CONFIG.lost;
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0f] flex flex-col">
        ...
        <div
          ...
          style={{
            background: isStolen
              ? 'linear-gradient(135deg, rgba(249,115,22,0.15) 0%, #0a0a0f 100%)'
              : 'linear-gradient(135deg, rgba(0,212,170,0.15) 0%, #0a0a0f 100%)',
          }}
        >
          <div className={`absolute inset-0 ${isStolen
            ? 'bg-[radial-gradient(ellipse_at_center,_#f9731615_0%,_transparent_70%)]'
            : 'bg-[radial-gradient(ellipse_at_center,_#00d4aa15_0%,_transparent_70%)]'
          }`} />
          <div className={`w-20 h-20 rounded-3xl flex items-center justify-center z-10 ${isStolen
        ...
        <div className="flex-1 px-6 -mt-6 relative z-10">
          ...
          <div className="bg-[#111118] rounded-3xl border border-white/5 p-6 mb-4">
            <h1 className="text-2xl font-bold text-white mb-2">{cta.successHeadline}</h1>
            <p className="text-slate-400 text-sm leading-relaxed mb-5">{cta.successSub}</p>
            ...
            <div className="bg-[#0a0a0f] rounded-2xl p-4 border border-white/5 mb-5">
              ...
              {isStolen && (
                <p className="text-orange-400/70 text-xs mt-1 flex items-center gap-1">
                  <Clock className="w-3 h-3" /> Reportado como roubado
                </p>
              )}
            </div>
            ...
            <div className="space-y-4">
              {cta.successTimeline.map((item, i) => (
                <div key={item.n} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div
                      ...
                      style={{
                        background: i === 0
                          ? (isStolen ? '#f97316' : '#00d4aa')
                          : 'rgba(255,255,255,0.08)',
                        color: i === 0 ? (isStolen ? 'white' : 'black') : 'rgba(255,255,255,0.5)',
                        border: i > 0 ? '1px solid rgba(255,255,255,0.12)' : 'none',
                      }}
                    >
                      {item.n}
                    </div>
                    {i < cta.successTimeline.length - 1 && (
          ...
          {/* Convite para criar conta — apenas quando faz sentido */}
          {cta.showChatInvite ? (
            <div className="rounded-3xl border border-[#00d4aa]/15 bg-[#00d4aa]/[0.04] p-5 mb-6">
              <p className="text-white font-semibold text-sm mb-1">Quer acompanhar a devolução?</p>
              <p className="text-slate-400 text-xs mb-4 leading-relaxed">
                Crie uma conta gratuita para receber atualizações e usar o chat protegido com o dono.
              </p>
              <Link
                href={`/auth/register?redirect=/scan/${code}`}
                className="flex items-center justify-center gap-2 bg-[#00d4aa] text-black font-bold px-5 py-3 rounded-xl text-sm w-full"
              >
              ...
              <Link
                href="/"
                className="flex items-center justify-center gap-2 mt-2 text-white/40 hover:text-white/60 text-xs transition-colors py-2"
              >
                Conhecer o Backfindr sem criar conta
                <ChevronRight className="w-3 h-3" />
              </Link>
            </div>
          ) : (
            <div className="rounded-3xl border border-orange-500/15 bg-orange-500/[0.04] p-5 text-center mb-6">
              <p className="text-slate-400 text-xs mb-3 leading-relaxed">
                Quer proteger seus próprios objetos com QR Code?
              </p>
              <Link
                href="/"
  ...
  return (
    ...
    <div className="min-h-screen bg-[#0a0a0f] flex flex-col" style={{ paddingBottom: '100px' }}>
      ...
      <div className="flex-1 px-5 pt-2">
        ...
        {/* Banner de alerta para objeto roubado */}
        {isStolen && (
          <div className="bg-orange-500/10 border border-orange-500/30 rounded-2xl p-4 mb-5 flex items-start gap-3">
            <Clock className="w-4 h-4 text-orange-400 flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-orange-300 text-sm font-semibold mb-0.5">Objeto reportado como roubado</p>
              <p className="text-orange-200/60 text-xs leading-relaxed">
                Se você viu ou está com este objeto, avise o dono anonimamente. Sua identidade não será revelada.
              </p>
            </div>
          </div>
        )}
      ...
      {/* ── CTA FIXO NA BASE — varia por status ── */}
      <div className="fixed bottom-0 inset-x-0 z-50 px-5 pb-8 pt-4"
        style={{ background: 'linear-gradient(to top, #0a0a0f 60%, transparent)' }}>
        <button
          onClick={notifyOwner}
          disabled={sending}
          className="w-full flex items-center justify-center gap-3 font-extrabold py-4.5 rounded-2xl text-base transition-all active:scale-95 disabled:opacity-70 shadow-2xl"
          style={{
            background: sending ? '#6b7280' : cta.buttonColor,
            paddingTop: '1.1rem',
            paddingBottom: '1.1rem',
            color: isStolen ? 'white' : 'black',
            boxShadow: isStolen
        ...
        >
          ...
          {sending ? 'Enviando...' : cta.buttonLabel}
        </button>
