...
export default function SucessoPage() {
  ...
  const [shareCount, setShareCount] = useState(0); // quantas vezes compartilhou
  const [justShared, setJustShared] = useState(false); // animação pós-share
  ...
  // Mensagens de incentivo progressivas por número de compartilhamentos
  const SHARE_NUDGES = [
  ...
  return (
    <div className="min-h-screen bg-[#080b0f] flex items-center justify-center px-5 py-10">
      ...
      <div className="w-full max-w-md relative z-10 text-center">
        ...
        {/* ══════════════════════════════════════════════════
            LOOP DE COMPARTILHAMENTO — WhatsApp principal
        ═══════════════════════════════════════════════════ */}
        <div className="mb-6">

          {/* Contador de grupos — aparece após 1º share */}
          {shareCount > 0 && (
            <div className={`flex items-center justify-center gap-2 mb-4 transition-all duration-500 ${justShared ? 'scale-110' : 'scale-100'}`}>
              ...
              <span className="text-white/50 text-xs">
                {shareCount === 1 ? '1 grupo atingido' : `${shareCount} grupos atingidos`}
              </span>
            </div>
          )}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-4 rounded-2xl font-bold text-base text-white transition-all mb-3 active:scale-95"
            style={{
          ...
          {/* Sub-texto após 1º share */}
          {shareCount > 0 && shareCount < 3 && (
            <p className="text-white/30 text-xs mb-3">
              {nudge.sub} — clique de novo para um grupo diferente
            </p>
          )}

          {/* Celebração após 3 grupos */}
          {shareCount >= 3 && (
            <div className="bg-teal-500/10 border border-teal-500/20 rounded-xl px-4 py-3 mb-3 text-center">
              <p className="text-teal-400 font-semibold text-sm mb-0.5">🎉 Incrível! 3 grupos alcançados</p>
              <p className="text-white/40 text-xs">Você maximizou as chances de recuperação. Parabéns!</p>
            </div>
          )}
      ...
      <div className="w-full max-w-md relative z-10 text-center">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-center gap-1.5 mb-3">
            <Sparkles className="w-3.5 h-3.5 text-teal-400" />
            <span className="text-teal-400 text-xs font-semibold uppercase tracking-wider">
              Compartilhe agora — aumenta muito as chances
            </span>
          </div>

          {/* Botão WhatsApp — tamanho máximo, destaque absoluto */}
          <a
            href={whatsappUrl}
            target="_blank"
