...
export default function PhoneFlowClient() {
  const router = useRouter();
  const [step, setStep] = useState(1);
  const [step1, setStep1] = useState<Step1Data>({ what: '', where: '', when: 'agora' });
  const [matches, setMatches] = useState<MatchItem[]>([]);
  const [loadingMatches, setLoadingMatches] = useState(false);

  useEffect(() => { analytics.flowStarted('phone'); }, []);
  useEffect(() => {
    const handleUnload = () => analytics.flowAbandoned('phone', step);
  ...
  function handleFinalize() {
    analytics.flowCompleted('phone', matches.length > 0);
    const params = new URLSearchParams();
    params.set('intent', 'lost');
    params.set('prefill_category', 'phone');
    params.set('prefill_title', ['Celular perdido', step1.what].filter(Boolean).join(' — '));
          ...
          <div className="pt-2">
            <div className="mb-7">
              <div className="inline-flex items-center gap-2 bg-red-500/10 border border-red-500/20 rounded-full px-3 py-1 mb-4">
                ...
                <span className="text-red-400 text-xs font-medium">Cada minuto conta — aja agora</span>
              </div>
              <h1 className="text-2xl font-bold text-white leading-tight mb-2">
                📱 Perdeu o celular?<br />Ainda dá tempo.
              </h1>
              <p className="text-white/40 text-sm leading-relaxed">
                Registre em 30 segundos. A rede começa a procurar imediatamente.
              </p>
            </div>

            <form onSubmit={handleStep1Submit} className="space-y-4">
              <div>
                <label className="block text-sm text-white/60 font-medium mb-1.5">Qual celular?</label>
                <input
                  type="text"
                  value={step1.what}
                  onChange={e => setStep1(s => ({ ...s, what: e.target.value }))}
                  placeholder="Ex: iPhone 14 preto, Samsung Galaxy azul..."
              ...
              <div>
                <label className="block text-sm text-white/60 font-medium mb-1.5 flex items-center gap-1.5">
                  <MapPin className="w-3.5 h-3.5 text-white/40" /> Onde foi?
                </label>
                <input
                  type="text"
                  value={step1.where}
                  onChange={e => setStep1(s => ({ ...s, where: e.target.value }))}
                  placeholder="Metrô, ônibus, restaurante, bairro..."
                  className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3.5 text-white placeholder-white/20 text-sm outline-none focus:border-teal-500/60 focus:bg-white/[0.06] transition-all"
                />
              </div>
              <div>
                <label className="block text-sm text-white/60 font-medium mb-2 flex items-center gap-1.5">
                  <Clock className="w-3.5 h-3.5 text-white/40" /> Quando?
                </label>
            ...
            {loadingMatches ? (
              <div className="flex flex-col items-center py-16 gap-4">
                ...
                <div className="text-center">
                  <p className="text-white font-semibold mb-1">Procurando seu celular...</p>
              ...
              <>
                <div className="mb-6">
                  <h2 className="text-2xl font-bold text-white leading-tight mb-2">
                    {matches.length > 0 ? 'Encontramos celulares parecidos.' : 'Nenhum resultado ainda.'}
                  </h2>
                  <p className="text-white/40 text-sm">
                    {matches.length > 0
                      ? `${matches.length} ${matches.length === 1 ? 'registro encontrado' : 'registros encontrados'} para "${step1.what}"`
                      : `Nenhum celular encontrado para "${step1.what}" — mas você pode publicar um alerta agora.`}
                  </p>
                </div>
                {matches.length > 0 && (
                  <div className="space-y-3 mb-6">
                    {matches.map(item => (
                      <FlowMatchCard key={item.id} item={item} onSelect={(i) => { analytics.flowMatchClicked('phone', i.unique_code || ''); router.push(`/objeto/${i.unique_code}`); }} label="Pode ser o meu" />
                    ))}
                  </div>
                )}
                {matches.length === 0 && (
                  <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5 mb-6 flex gap-3">
                    ...
                    <div>
                      <p className="text-white/70 text-sm font-medium mb-1">A rede fica de olho.</p>
                      <p className="text-white/40 text-xs leading-relaxed">Se alguém reportar um celular parecido, você é avisado imediatamente.</p>
                    </div>
                  </div>
                )}
                <div className="space-y-3">
                  <button onClick={() => { analytics.flowStep('phone', 3, 3); setStep(3); }}
        ...
        <FlowLayout step={3} totalSteps={3}>
          <div className="pt-2">
            <div className="mb-7">
              ...
              <p className="text-white/40 text-sm leading-relaxed">Conta gratuita em segundos. Se seu celular aparecer, você recebe notificação na hora.</p>
            </div>
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-4 mb-6 space-y-2">
              <div className="flex items-center gap-2 text-sm"><span className="text-white/40 w-20 flex-shrink-0">Categoria:</span><span className="text-white font-medium">📱 Celular</span></div>
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => { const text = `📱 Perdi meu celular${step1.what ? ` — ${step1.what}` : ''}${step1.where ? ` em ${step1.where}` : ''}. Alguém viu? backfindr.com/flow/phone`; if (navigator.share) navigator.share({ title: 'Perdi meu celular', text }); else window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank'); }}
